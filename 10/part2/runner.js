...
const countTrails = (data, currentPosition, currentValue, bounds, path) => {
    ...
    if (inBounds(nextPosition, bounds)) {
      ...
      if (nextValue === currentValue + 1) {
        ...
        if (nextValue === 9) {
          ...
          fullPaths.push(
            ...countTrails(data, nextPosition, nextValue, bounds, nPath)
          );
        }
      }
    }
...
const startAtTrailHeads = (data) => {
  ...
  for (let y = 0; y < bounds.maxY; y++) {
    for (let x = 0; x < bounds.maxX; x++) {
      if (data[y][x] === 0) {
        const paths = countTrails(data, { x, y }, 0, bounds, [0]);
        if (paths.length) {
          total += paths.length;
        }
      }
